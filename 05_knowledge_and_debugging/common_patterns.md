...
### Pattern 22: DVB COMPUTE Section Not Found (TTL NB Builds)
- **Phase:** BUILD
- **Symptoms:** `Compute section not found`, `getLf: error`, `Exit Status 2` on jem/vcssimmpp/cpp, genFlow NB sub-tasks fail
- **Description:** DVB's `genFlow.py` calls `LiteInfraEnv.get_compute_cfg()` → `Config().get(section="compute=")` to determine NB routing (pool, qslot, class) for per-lib sub-tasks. If `cfg/compute.cth` is empty and no COMPUTE section is found, sub-tasks submit without `--class`/`--qslot` and land in the wrong NB pool.
- **Key Distinction:** `cfg/compute.cth` is **intentionally empty in the TTL seed**. Do NOT manually fill it in. The `[INCLUDES]` pointing to site_config (`/nfs/site/disks/issp_ttl_emu_compile_001/site_config/compute.ttl.zsc16.SLES15.cth`) is auto-generated by DVB infrastructure during normal flow.
- **Root Cause in Copilot environment:** The real blocker is usually `LM_PROJECT` being invalid (`SC_HNGUY11_UNKN`) — not the COMPUTE section. Fix LM_PROJECT first.
- **General Fix:** 
  1. Ensure `export LM_PROJECT=DDG-TTLPKG` before grdlbuild
  2. Ensure `useNBResource("MEM512G_4C")` is present in the task definition in `flows/grdlbuild/ttlbx_n2p/emu/sle/build.gradle.kts` — this routes the Layer 1 grdlbuild wrapper job via `resources.ini`
  3. Do NOT modify `cfg/compute.cth` — leave it empty
- **DVB CTH Format Notes:**
  - DVB's own `common/tool.cth` uses `[ compute ]` (lowercase, spaces) format
  - The verbose `[COMPUTE]` (uppercase, no spaces) and concise `[COMPUTE jem@zsc16]` formats are NOT interchangeable with what `get_compute_cfg()` expects
  - The site_config file uses `[COMPUTE]` + `Name = jem@zsc16` format
- **Related:** resources.ini, build.gradle.kts useNBResource(), LM_PROJECT

### Pattern 23: resources.ini Layer 1 NB Routing (TTL)
- **Phase:** BUILD
- **Symptoms:** NB jobs land in wrong pool, `grdlbuild -nb` tasks get wrong class/qslot
- **Description:** `flows/grdlbuild/resources.ini` controls Layer 1 NB routing for grdlbuild wrapper tasks. The `[global] default` entry provides base routing (`p=`, `q=`, `c=`). Named resources (like `MEM512G_4C`) override only the `c=` class component, inheriting `p=` and `q=` from default.
- **Key Values (TTL zsc16):**
  - Default: `p=zsc16_express,q=/PCH/CSS/TTL/emu,c=SLES15&&64G`
  - `MEM512G_4C`: `c=SLES15&&512G&&4C` → full routing becomes `p=zsc16_express,q=/PCH/CSS/TTL/emu,c=SLES15&&512G&&4C`
- **How to use:** Add `useNBResource("MEM512G_4C")` to the task definition in `build.gradle.kts`
- **File locations:**
